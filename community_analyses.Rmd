...
First, let's scale the data - change the raw scores to z-scores.
...
Now lets do a kmeans analysis and use the elbow method to see if there are optimal clusters.
...
Not sure if there is a clear number of clusters. Lets try using BIC
...
plot(d_clust)
```
Lets goe a different route and use NbClust to see if we can figure out how many clusters there might be.
