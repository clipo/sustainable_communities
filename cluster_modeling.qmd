...
-   Update data in CSV to include just the raw values for the columns we use

-   Remove the code that selects a subset of columns

-   Trim the various bits of code that produces figures we don't need or use.
...
Here, we read in the data we want to analyze. At the moment, the file we have includes raw values, so we need to turn them into z-scores to standardize everything. We are using 2015 data. Ideally, we will regenerate these values for 2022 (or at least 2020).
...
First we want to do some basic work with principle components. We generate the output of the first 17 PCs. Then, we generate the loadings of the PCs for each community in a CSV file for analyses.
...
Now lets look at the how the PCs are composed.
