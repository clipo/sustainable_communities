...
spplot(merged_clean, "x_f")#, col.regions=brewer.pal(8,'Set1'))
...
library(leaflet)
pal <- colorFactor(rainbow(8), merged_clean$x,
                   na.color = "transparent")
p_popup <- paste0("<strong>Cluster: </strong>", merged_clean$x)
leaflet(merged_clean) %>%
  addPolygons(
...
pal <- colorFactor(rainbow(8), merged_clean$x,
                   na.color = "transparent")
p_popup <- paste0("<strong>MSA: </strong>", merged_clean$NAME)
leaflet(merged_clean) %>%
  addPolygons(
    stroke = FALSE, # remove polygon borders
    fillColor = ~pal(x), # set fill color with function from above and value
